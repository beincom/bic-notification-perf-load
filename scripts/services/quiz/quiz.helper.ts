...
import dumps from './dumps/quiz_dump.json';

export class QuizHelper {
  public static async convertQuizDumpToArray(): Promise<void> {
    console.log({ step: 'START' });

    const quizzes = (dumps as any[]).reduce((acc, quiz) => {
    ...
    writeFileSync('scripts/services/quiz/dumps/quiz_array.json', JSON.stringify(quizzes, null, 2));
